<?php
	...
	$objh->customer_gender = trim($rs->fields['customer_gender']);
	$objh->customer_birthdate = SQLUTIL::SQLDateParseToStringdatesmall(trim($rs->fields['customer_birthdate']));
	$objh->customer_withbirthdate = trim($rs->fields['customer_withbirthdate']);
	...
	$objh->customer_isvalid = trim($rs->fields['customer_isvalid']);
	$objh->customer_ismega = trim($rs->fields['customer_ismega']);
	...
	$objh->customer_gender = trim($rs->fields['gender_id']);
	...
	*/	
	...
	$sql = "select * from master_customerpurchase where customer_id='$id'";
	$rs  = $conn->Execute($sql);
	unset($arrdata);
	$arrdata = array();
	while (!$rs->EOF) {
		unset($obj);
		$obj->customer_id = trim($rs->fields['customer_id']);
		$obj->customerpurchase_line = $rs->fields['customerpurchase_line'];
		$obj->customerpurchase_date = $rs->fields['customerpurchase_date'];
		$obj->customerpurchase_descr = $rs->fields['customerpurchase_descr'];
		$obj->customerpurchase_qty = 1*$rs->fields['customerpurchase_qty'];
		$obj->customerpurchase_value= 1*$rs->fields['customerpurchase_value'];
	...
	$sql_R = sprintf("select region_name from master_region where region_id = '%s'", $obj->region_id);
	$rs_R = $conn->Execute($sql_R);
	$obj->region_name = trim($rs_R->fields['region_name']);
 
 
 
 	$obj->branch_id= $rs->fields['branch_id'];
 	$sql_B = sprintf("select branch_name from master_branch where branch_id = '%s'", $obj->branch_id);
	$rs_B = $conn->Execute($sql_B);
	$obj->branch_name = trim($rs_B->fields['branch_name']);
	...
	$data[0]['D']['DetilPurchase'] = $arrdata;
