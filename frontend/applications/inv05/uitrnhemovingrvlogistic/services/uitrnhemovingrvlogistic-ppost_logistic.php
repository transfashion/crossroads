		...
		if ($__ACTION=='POSTALL' || $__ACTION=='UNPOSTALL') {
			if ($__ACTION=='UNPOSTALL') {
				...
				if ($cannotunpost) {
					...
					unset($obj);
					$obj->hemoving_islogisticpost = 0;
					$obj->hemoving_logisticpostby = '';
					$obj->hemoving_logisticpostdate = '__DBNULL__';		
					...
					$POSTTYPE = "LOGISTICPOST";
					$failed = 0;
					$POSTVALUE = 1;
					$POSTMSG   = "Data '$__ID' sudah di-post";
					$criteria = sprintf("%s='%s'", $__CONF['H']['PRIMARY_KEY'], $__ID);
					unset($obj);
					$obj->hemoving_islogisticpost = 1;
					$obj->hemoving_logisticpostby = $username;
					$obj->hemoving_logisticpostdate = SQLUTIL::SQL_GetNowDate();					
					$SQL = SQLUTIL::SQL_UpdateFromObject($__CONF['H']['TABLE_NAME'], $obj, $criteria);
					$conn->Execute($SQL);
