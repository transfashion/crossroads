...
if (!defined('__SERVICE__')) {
	...
	/* tambahan logistic
	   ...
	   28/10/2011
	*/
	$objh->hemoving_sn = trim($rs->fields['hemoving_sn']);
	$objh->hemoving_pol = trim($rs->fields['hemoving_pol']) ? trim($rs->fields['hemoving_pol']) : "0";
	$objh->hemoving_etd = SQLUTIL::SQLDateParseToStringdatesmall($rs->fields['hemoving_etd']);
	$objh->hemoving_eta = SQLUTIL::SQLDateParseToStringdatesmall($rs->fields['hemoving_eta']);
	$objh->hemoving_logisticcosttmp = trim($rs->fields['hemoving_logisticcosttmp']) ? trim($rs->fields['hemoving_logisticcosttmp']) : "A1";
   $objh->hemoving_islogisticpost = trim($rs->fields['hemoving_islogisticpost']);
   $objh->hemoving_logisticpostby = trim($rs->fields['hemoving_islogisticpost']);
   $objh->hemoving_logisticpostdate = trim($rs->fields['hemoving_logisticpostdate']);
	/* end tambahan logistic */

	$objh->hemoving_isprop = $rs->fields['hemoving_isprop'];
	$objh->hemoving_isproplock = $rs->fields['hemoving_isproplock'];
	...
	$objh->hemoving_proplockby = $rs->fields['hemoving_proplockby'];
	...
	$sql  = "select A.*, _heinv_art=B.heinv_art, _heinv_mat=B.heinv_mat, _heinv_col=B.heinv_col, _heinv_name=B.heinv_name, heinvgro_id=B.heinvgro_id, heinvctg_id=B.heinvctg_id, heinv_sizetag=(SELECT heinvctg_sizetag FROM master_heinvctg WHERE heinvctg_id=B.heinvctg_id and region_id = B.region_id) ";
	$sql .= "from ";
	...
	while (!$rs->EOF) {
		...
		$obj->heinv_art = $rs->fields['_heinv_art'];
		$obj->heinv_mat = $rs->fields['_heinv_mat'];
		$obj->heinv_col = $rs->fields['_heinv_col'];
		$obj->heinv_name = $rs->fields['_heinv_name'];
	...
	unset($arrdata);
	$sql = "SELECT * FROM transaksi_hemovinginvoice where hemoving_id='$id' ";
	$rs  = $conn->Execute($sql);
	$arrdata = array();
	while (!$rs->EOF) {
		unset($obj);
		$obj->hemoving_id						= $rs->fields['hemoving_id'];	
		$obj->hemovinginvoice_line			= (int) $rs->fields['hemovinginvoice_line'];
		$obj->hemovinginvoice_ref			= $rs->fields['hemovinginvoice_ref'];
		$obj->hemovinginvoice_date			= $rs->fields['hemovinginvoice_date'];
		$obj->hemovinginvoice_datedue		= $rs->fields['hemovinginvoice_datedue'];
		$obj->hemovinginvoice_descr		= $rs->fields['hemovinginvoice_descr'];
		$obj->hemovinginvoice_qty			= (int) $rs->fields['hemovinginvoice_qty'];
		$obj->hemovinginvoice_foreign		= (float) $rs->fields['hemovinginvoice_foreign'];
		$obj->hemovinginvoice_foreignrate= (float) $rs->fields['hemovinginvoice_foreignrate'];	
		$obj->hemovinginvoice_idr			= (float) $rs->fields['hemovinginvoice_idr'];
		$obj->currency_id						= $rs->fields['currency_id'];
		$obj->rowid                      = $rs->fields['rowid'];
		$arrdata[] = $obj;
		$rs->MoveNext();
	}		
	$data[0]['D']['DetilInvoice'] = $arrdata;



	unset($arrdata);
	$sql = "SELECT * FROM transaksi_hemovinglogisticcost where hemoving_id='$id' ";
	$rs  = $conn->Execute($sql);
	$arrdata = array();
	while (!$rs->EOF) {
		unset($obj);
		$obj->hemoving_id						= $rs->fields['hemoving_id'];
		$obj->hemovinglogisticcost_line  = (int) $rs->fields['hemovinglogisticcost_line'];
		$obj->hemovinglogisticcost_descr = $rs->fields['hemovinglogisticcost_descr'];
		$obj->hemovinglogisticcost_amount= (float) $rs->fields['hemovinglogisticcost_amount'];
		$obj->hemovinglogisticcost_rate	= (float) $rs->fields['hemovinglogisticcost_rate'];
		$obj->acclogisticcost_id			= $rs->fields['acclogisticcost_id'];
	...
	$data[0]['D']['DetilLogisticCost'] = $arrdata;
	...
	$sql = "select rekanan_name from master_rekanan where rekanan_id='".$objh->hemoving_pol."'";
	$rs  = $conn->Execute($sql);
	$objh->hemoving_polname = $rs->fields['rekanan_name'];
