...
if (!defined('__SERVICE__')) {
	...
	$sql = "select * from transaksi_custtraffic where custtraffic_id ='$__ID'";
	$rs  = $conn->Execute($sql);
	$custtraffic_id = $rs->fields['custtraffic_id'];		
	...
	try {
		...
		if ($__ACTION=='POST' || $__ACTION=='UNPOST') {
			...
			if (!$failed) {
				...
				$obj->log_table		= "transaksi_custtraffic";
