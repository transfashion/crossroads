<?php
	...
	try {
		...
		$conn->Execute(sprintf("DELETE FROM %s WHERE %s='%s'", $__CONF['D']['DetilException']['TABLE_NAME'], $__CONF['D']['DetilException']['PRIMARY_KEY1'], $__ID));
		$conn->Execute(sprintf("DELETE FROM %s WHERE %s='%s'", $__CONF['D']['DetilItem']['TABLE_NAME'], $__CONF['D']['DetilItem']['PRIMARY_KEY1'], $__ID));
