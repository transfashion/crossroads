...
from dbo.cache_heinvsummary WHERE cacheid='$cacheid' ORDER BY REPSEQ, heinvctg_name, heinvctg_id, REPSECTIONSEQ";
	//$rs = $conn->Execute($sql);
	...
	while (!$rs->EOF) {
		...
		$obj->heinv_sizetag = $rs->fields['heinv_sizetag']; 
               ...
               {
               $obj->heinv_price01 =number_format((float) $rs->fields['heinv_price01'],0); 
		$obj->heinv_pricedisc01 =number_format( (float) $rs->fields['heinv_pricedisc01'],0); 
                }
                else
               {
                 $obj->heinv_price01 ='';
