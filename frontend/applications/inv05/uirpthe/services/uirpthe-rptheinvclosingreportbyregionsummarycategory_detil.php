...
if (!defined('__SERVICE__')) {
	...
	$data = array();
		...
		$sql = "
		SET NOCOUNT ON
		EXEC inv05_RptClosingReport_RegionSummaryCategory '$region_id','$periode_id'  ";
	$rs  = $conn->Execute($sql);
	...
	while (!$rs->EOF) {
		...
		$obj->pricesa			=	(float) trim($rs->fields['PRICESA']);
		$obj->discsa			=	(float) trim($rs->fields['DISCSA']);
		$obj->nettsa			=	(float) trim($rs->fields['NETTSA']);
		...
		$obj->ID_group1			=	'';
		$obj->ID_group2			=	'';
		$obj->ID_detil			=	trim($rs->fields['heinvctg_name']);;
