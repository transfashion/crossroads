<?php
		...
		if (is_array($arrDetilData)) {
			for ($i=0; $i<count($arrDetilData); $i++) {
				...
				$heinv_id =   $arrDetilData[$i]->heinv_id;
				$objM->heinv_id = $heinv_id;
				$objM->heinv_isassembly = 1;
				
				$SQLM = "
				SELECT A.hemoving_id  from transaksi_hemoving A inner join transaksi_hemovingdetil B on A.hemoving_id = B.hemoving_id
AND A.hemoving_ispost=1 AND LEFT(A.hemoving_id,2) = 'AM' and heinv_id = '$heinv_id'";

				$rsM = $conn->execute($SQLM);
				$objM->ref_id = $rsM->fields['hemoving_id'];				
				
				$SQLM = SQLUTIL::SQL_UpdateFromObject("master_heinv", $objM,"heinv_id = '$heinv_id'");
				$conn->execute($SQLM);
