...
/*
	...
	$sql .= "where hemovingdetil_line hemoving_id='$id'";
	$rs  = $conn->Execute($sql);
*/
...
A.hemoving_descr,
A.region_id,
heinvgro_id = C.heinvgro_id,
heinvctg_id = C.heinvctg_id,
heinv_sizetag=(SELECT heinvctg_sizetag FROM master_heinvctg WHERE heinvctg_id=C.heinvctg_id and region_id = C.region_id),
...
inner join master_heinv C on B.heinv_id = C.heinv_id

WHERE A.hemoving_id = '$id' 
GROUP BY 
A.hemoving_id,  
B.hemovingdetil_line,
B.heinv_id,
...
;
	...
	while (!$rs->EOF) {
		...
		$obj->heinv_subtotal = 1*$obj->heinv_qtyrecv*$obj->heinv_price;
	...
	$arrdataItem[] = $obj;
	$data[0]['D']['DetilItemFG'] = $arrdataItem;
...
;
	...
	$data[0]['D']['DetilItemRM'] = $arrdataItem;
