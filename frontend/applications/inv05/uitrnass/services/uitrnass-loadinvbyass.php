...
if (!defined('__SERVICE__')) {
	...
	if (is_array($objCriteria)) {
		...
		$region_id = SQLUTIL::BuildCriteria($param, $CRITERIA_DB, 'obj_search_chk_region_id', 'A.region_id', "{criteria_value}");
		$heinv_art = SQLUTIL::BuildCriteria(&$SQL_CRITERIA, $CRITERIA_DB, 'obj_search_txt_heinvart', 'C.heinv_art',"{db_field}  = '{criteria_value}'");
		$heinv_id = SQLUTIL::BuildCriteria(&$SQL_CRITERIA, $CRITERIA_DB, 'obj_search_txt_tmid', 'B.heinv_id', "{db_field}  = '{criteria_value}'");
		$heinv_descr = SQLUTIL::BuildCriteria(&$SQL_CRITERIA, $CRITERIA_DB, 'obj_search_txt_descr', 'C.heinv_descr', "{db_field} LIKE '%{criteria_value}%' ");
		$hemoving_id = SQLUTIL::BuildCriteria(&$SQL_CRITERIA, $CRITERIA_DB, 'obj_search_txt_ass', 'A.hemoving_id', "{db_field}  = '{criteria_value}'");
	}
...
WHERE $SQL_CRITERIA AND left(A.hemoving_id,2)='AM' and A.hemoving_ispost=1
GROUP BY 
A.hemoving_id,  
B.hemovingdetil_line,
B.heinv_id,
