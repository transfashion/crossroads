...
$sql = "SELECT * FROM master_branch where branch_type in('HO','WH')";
$rsUser  = $conn->execute($sql);
while (!$rsUser->EOF)
{
	unset($obj);
    $obj->branch_id = $rsUser->fields['branch_id'];
	$obj->branch_name = $rsUser->fields['branch_name'];
	$obj->branch_type = $rsUser->fields['branch_type'];
    $data[] = $obj;
    $rsUser->MoveNext();
}
