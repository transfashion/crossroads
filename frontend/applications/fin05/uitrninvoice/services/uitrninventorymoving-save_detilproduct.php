<?php
		...
		if (is_array($arrDetilData)) {
			for ($i=0; $i<count($arrDetilData); $i++) {
				...
				$obj->inventorymovingdetil_qtypropose	= $arrDetilData[$i]->inventorymovingdetil_qty;
				$obj->inventorymovingdetil_qtyinit		= $arrDetilData[$i]->inventorymovingdetil_qty;
