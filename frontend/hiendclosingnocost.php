...
BEGIN
	...
	DECLARE @_day AS int;
	DECLARE @_month AS int;
	DECLARE @_year AS int
	DECLARE @_dt_first_nextmonth AS smalldatetime;
	DECLARE @_dt_last_prevmonth AS smalldatetime;
	DECLARE @heinvclosingstatus_id_lastmonth AS varchar(12);
	DECLARE @heinvclosingstatus_id AS varchar(12);
	DECLARE @saldo_id_pref AS VARCHAR(30);

	
	SET @_dt_first_nextmonth = DATEADD(MONTH, 1, @date);
	SET @_dt_first_nextmonth = CAST((CAST(YEAR(@_dt_first_nextmonth) as varchar) + '-' + CAST(MONTH(@_dt_first_nextmonth) as varchar) + '-1') as smalldatetime);
	SET @_day = DAY(DATEADD(DAY, -1, @_dt_first_nextmonth))
	SET @_month = MONTH(@date);
	SET @_year = YEAR(@date);
	SET @_dt_last_prevmonth = DATEADD(dd, -1, CAST((CAST(@_year as varchar) + '-' + dbo.f_zerofill(@_month, 2) + '-01') as smalldatetime))
	SET @heinvclosingstatus_id_lastmonth =   cast(YEAR(@_dt_last_prevmonth) as varchar) + dbo.f_zerofill(MONTH(@_dt_last_prevmonth), 2) + '-' + @region_id;
	SET @heinvclosingstatus_id = cast(YEAR(@date) as varchar) + dbo.f_zerofill(MONTH(@date), 2) + '-' + @region_id;	
	SET @saldo_id_pref = cast(YEAR(@date) as varchar) + dbo.f_zerofill(MONTH(@date), 2)  + dbo.f_zerofill(DAY(@date), 2) + '-' + @region_id + '-%';	

	--SELECT @heinvclosingstatus_id, @saldo_id_pref, @heinvclosingstatus_id_lastmonth


	-- DATA CLOSING BULAN LALU
	SELECT heinvclosingstatus_id, heinv_id, LASTCOST=COST 
	INTO #TEMP_LASTMONTH
	FROM transaksi_heinvclosingstatusdetil WHERE heinvclosingstatus_id=@heinvclosingstatus_id_lastmonth

	-- DATA CLOSING BULAN INI
	SELECT  heinvclosingstatus_id, heinv_id, 
	RV, RV_VAL,
	RVUNIT = CAST((CASE WHEN RV<>0 THEN RV_VAL/RV ELSE 0 END) as decimal(18,2)),
	COST
	INTO #TEMP_CURRMONTH
	FROM transaksi_heinvclosingstatusdetil WHERE heinvclosingstatus_id=@heinvclosingstatus_id
	...
	SELECT 
	A.heinv_id,
	B.heinv_art, B.heinv_mat, B.heinv_col, B.heinv_name, B.season_id,
	branch_id = substring(A.saldo_id, 16,7),
	LASTCOST = isnull((SELECT LASTCOST FROM #TEMP_LASTMONTH WHERE heinv_id=A.heinv_id ), 0),
	RVUNIT = isnull((SELECT RVUNIT FROM #TEMP_CURRMONTH WHERE heinv_id=A.heinv_id ), 0),
	COST = isnull((SELECT COST FROM #TEMP_CURRMONTH WHERE heinv_id=A.heinv_id ), 0),
	[BEG] = A.saldodetil_begin,
	[RV]  = A.saldodetil_recv,
	[TRO] = A.saldodetil_trou,
	[TRI] = A.saldodetil_trin,
	[TRT] = A.saldodetil_trtran,
	[SL]  = A.saldodetil_sales,
	[DO]  = A.saldodetil_do,
	[AJ]  = A.saldodetil_adj,
	[ASO] = CASE WHEN A.saldodetil_as < 0 THEN A.saldodetil_as ELSE 0 END,
	[ASI] = CASE WHEN A.saldodetil_as > 0 THEN A.saldodetil_as ELSE 0 END,
	[OTH] = A.saldodetil_oth,
	[END] = A.saldodetil_end,
	[ENDVAL] = A.saldodetil_endvalue,
	B.heinvctg_id
	INTO #TEMP_THISMONTH
	FROM dbo.transaksi_hesaldodetil A inner join master_heinv B on A.heinv_id = B.heinv_id
	WHERE A.saldo_id LIKE @saldo_id_pref
	


	SELECT distinct heinv_id 
	INTO #TEMP_AX
	FROM #TEMP_THISMONTH
	WHERE
	COST=0 AND SL<>0
...
SELECT 
heinv_id,
heinv_art = (select heinv_art from master_heinv where heinv_id=A.heinv_id),
heinv_mat = (select heinv_mat from master_heinv where heinv_id=A.heinv_id),
heinv_col =(select heinv_col from master_heinv where heinv_id=A.heinv_id),
heinv_name = (select heinv_name from master_heinv where heinv_id=A.heinv_id),
COST = isnull((SELECT COST FROM #TEMP_CURRMONTH WHERE heinv_id=A.heinv_id ), 0),
qty = isnull(sum(bondetil_qty), 0),
gross = isnull(SUM(itemgrossori), 0),
nett = isnull(SUM(nett), 0)
FROM view_hepos_bonlist_1 A where 
        bon_isvoid=0
	AND YEAR(bon_date)  = YEAR(@date)
	AND MONTH(bon_date) = MONTH(@date)
	AND DAY(bon_date) <= DAY(@date)
	AND region_id = @region_id
    --AND branch_id NOT IN ('0004100')
	AND heinv_id IN (SELECT heinv_id FROM #TEMP_AX)
GROUP BY 
A.heinv_id


	DROP TABLE #TEMP_LASTMONTH;
	DROP TABLE #TEMP_CURRMONTH;
	DROP TABLE #TEMP_THISMONTH;
	DROP TABLE #TEMP_AX;
